<div class="relative">
    ...
    <nav x-data="{ showSidebar: false, showMenu: true }" @scroll.window="showSidebar = (window.pageYOffset > window.innerHeight) ? true : false"
        ...
        class="z-50 hidden shrink-0 grow-0 justify-around gap-2 border-t p-1.5 shadow-lg backdrop-blur-lg fixed top-2/4 opacity-90 hover:opacity-100
    -translate-y-2/4 left-0 min-h-[auto] flex-col rounded-r-xl border border-l-0 border-gray-500 hover:shadow-2xl overflow-hidden">

        <div class="flex flex-col justify-around gap-2 items-center w-8 h-8  rounded group">

            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="cursor-pointer  w-5 h-5" @click="showMenu = !showMenu"
                :class="(showMenu === true) ? '' : 'hidden'">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />
            </svg>

            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="cursor-pointer w-6 h-6" @click="showMenu = !showMenu"
                :class="(showMenu === true) ? 'hidden' : ''">
